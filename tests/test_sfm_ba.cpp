...
#define NIMGS_LIMIT                           20 // сколько фотографий обрабатывать (можно выставить меньше чтобы ускорить экспериментирование, или в случае если весь датасет не выравнивается)
...
public:
    ...
    bool operator()(const T* camera_extrinsics, // положение камеры:   [6] = {translation[3], rotation[3]} (разное для всех кадров, т.к. каждая фотография со своего ракурса)
        ...
        for (size_t i = 0; i < 3; ++i)
            p[i] = point_global[i] - camera_extrinsics[i];
        ...
        {
            T p_rotated[3];
            ceres::AngleAxisRotatePoint(camera_extrinsics + 3, p, p_rotated);
            std::swap(p, p_rotated);
        }

        // Проецируем точку на фокальную плоскость матрицы (т.е. плоскость Z=фокальная длина), тем самым переводя в пиксели
        T x = camera_intrinsics[2] * p[0] / p[2];
        T y = camera_intrinsics[2] * p[1] / p[2];

#if ENABLE_INSTRINSICS_K1_K2
        // k1, k2 - коэффициенты радиального искажения (radial distortion)
        const T r2 = x * x + y * y;
        const T l = T(1) + (camera_intrinsics[0] + camera_intrinsics[1] * r2) * r2;
           ...
           bool verbose)
{
    ...
    double camera_intrinsics[5] {
        calib.k1_, calib.k2_, calib.f_, calib.cx_ + 0.5 * calib.width(), calib.cy_ + 0.5 * calib.height()
    };
    std::cout << "Before BA ";
    ...
    for (size_t i = 0; i < tie_points.size(); ++i) {
        ...
        for (size_t ci = 0; ci < track.img_kpt_pairs.size(); ++ci) {
            ...
            if (ENABLE_OUTLIERS_FILTRATION_COLINEAR && ENABLE_BA && ci > 0) {
                // проверка случая когда два луча почти параллельны, чтобы не было странных точек улетающих на бесконечность (например чтобы угол был хотя бы 2.5 градуса)
                static constexpr double min_delta = 2.5 / 180 * M_PI;  // 2.5° to radians 
                static constexpr double max_cos = std::cos(min_delta); // angle >= delta -> cos <= cos(delta)
                ...
                should_be_disabled = ray_first.dot(ray_current) > max_cos;
            }
