...
namespace {
    ...
    cv::Matx34d estimateCameraMatrixDLT(const cv::Vec3d *Xs, const cv::Vec3d *xs, int count)
    {
        ...
        Eigen::VectorXd right_null_space = svd.matrixV().col(11);

        matrix34d result(right_null_space[0], right_null_space[1], right_null_space[2], right_null_space[3],
                         right_null_space[4], right_null_space[5], right_null_space[6], right_null_space[7],
                         right_null_space[8], right_null_space[9], right_null_space[10], right_null_space[11]);
    ...
    cv::Matx34d estimateCameraMatrixRANSAC(const phg::Calibration &calib, const std::vector<cv::Vec3d> &X, const std::vector<cv::Vec2d> &x)
    {
        ...
        for (int i_trial = 0; i_trial < n_trials; ++i_trial) {
            ...
            for (int i = 0; i < n_points; ++i) {
                cv::Vec3d projected = calib.project(P * vector4d{X[i][0], X[i][1], X[i][2], 1});
                cv::Vec2d px = {projected[0] / projected[2], projected[1] / projected[2]} ; //спроецировать 3Д точку в пиксель с использованием P и calib;
