...
TEST (CeresSolver, HelloWorld1) {
    ...
    // TODO 1 -- done: почему результирующая производная не ноль? мы ведь должны были сойтись в минимуме функции 0.5*(10-x)^2
    //
...
public:
    ...
    bool operator()(const T* const queryPoint, T* residual) const {
        ...
        residual[0] = a*dx*dx + b*dy*dy - queryPoint[2] + center[2] ;
        return true;
    }
protected:
    double center[3];
...
TEST (CeresSolver, HelloWorld2) {
    ...
    // TODO 2 -- done удалите эту строку, затем
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // TODO 3-- done: раскомментируйте^, почему он находит не то что ожидалось? -- не учитывается координата z при вычислении ошибки расстояния до параболы
...
public:
    ...
    bool operator()(const T* const line, T* residual) const {
        ...
        T l2norm = ceres::sqrt(line[0]*line[0] + line[1]*line[1]);
        residual[0] = (line[0]*(T)samplePoint[0] + line[1]*(T)samplePoint[1] + line[2])/l2norm;
        return true;
    }
protected:
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    for (size_t i = 0; i < n_points; ++i) {
        ...
        // TODO 6 -- done удалите этот return сразу после выполнения TODO 5
    ...
    // preproccesing (постобработка)
    double trans = ideal_line[2]/line_params[2]; // выбор пал на свободный член, потому что он самый большой по модулю(меньше ошибка из-за чисел с плавающей точкой)
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // Расходятся потому что прямые равны, но коэффициенты у прямых равны coef(ideal) = k * coef(line) - с точностью до константы
        ...
        // TODO 7 -- done поправьте тест так или иначе (хотя бы пост-процессингом)
        // Исправить препроцессингом можно приводя к одному и тому же виду(найти k из уравнения выше)
        // Исправить в формулировке задачи можно убрав(зафиксировав) одну из искомых коэффициентов
    }

    // Оцениваем качество идеальной прямой
    double inliers_fraction, mse;
    evaluateLine(points, ideal_line, sigma, inliers_fraction, mse);
    ASSERT_GT(inliers_fraction, 0.99 - outliers_fraction); // TODO 8 -- done раскоментируйте, почему эта проверка падает? как поправить?
//    ASSERT_LT(mse, 1.1 * sigma * sigma); // TODO 9 -- done раскомментируйте, почему проверка падает? на каких тестах она падает, на каких проходит? попробуйте отладить рассчет mse_inliers_distance в evaluateLine
    ...
    if (outliers_fraction == 0 || use_huber) {
        // TODO 10 -- done раскоментируйте обе проверки, почему они падают? в каких тестах? поправьте (в т.ч. подобно тому как было с ослаблением порога выше)
