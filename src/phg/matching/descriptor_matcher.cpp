                                                    ...
                                                    std::vector<cv::DMatch> &filtered_matches)
{
    ...
    for (int i = 0; i < matches.size(); ++i) {
        if (matches[i].size() >= 2) {
            if (matches[i][0].distance <= MAX_RATIO * matches[i][1].distance) {
                                                   ...
                                                   std::vector<cv::DMatch> &filtered_matches)
{
    ...
    std::shared_ptr<cv::flann::IndexParams> index_params = flannKdTreeIndexParams();
    std::shared_ptr<cv::flann::SearchParams> search_params = flannKsTreeSearchParams();
...
//    метч остается, если левое и правое множества первых total_neighbors соседей в радиусах поиска(radius2_query, radius2_train) имеют как минимум consistent_matches общих элементов
    ...
    int j = 0;
    for (int i = 0; i < matches.size(); ++i) {
        std::set<int> indices_query_local;
        std::set<int> indices_train_local;

        j = 0;
        while (indices_query_local.size() < total_neighbours && distances2_query.at<float>(i, j) < radius2_query ) {
            indices_query_local.insert(indices_query.at<int>(i, j));
            j++;
        }
        j = 0;
        while (indices_train_local.size() < total_neighbours && distances2_train.at<float>(i, j) < radius2_train ) {
            indices_train_local.insert(indices_train.at<int>(i, j));
            j++;
        }

        std::vector<int> indices_intersection;
        std::set_intersection(indices_query_local.begin(), indices_query_local.end(),
                              indices_train_local.begin(), indices_train_local.end(),
                              std::back_inserter(indices_intersection));
        if (indices_intersection.size() >= consistent_matches) {
