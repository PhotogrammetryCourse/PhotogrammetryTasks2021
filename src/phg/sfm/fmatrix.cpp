...
//             Поправить F так, чтобы соблюдалось свойство фундаментальной матрицы (последнее сингулярное значение = 0)
        ...
        S.diagonal() << svdf.singularValues()(0), svdf.singularValues()(1), 0;
    ...
    {
        cv::Scalar_<double> mean, std_xy;
        cv::meanStdDev(m, mean, std_xy);
        double std = sqrt(std_xy(0)* std_xy(0) + std_xy(1) * std_xy(1)) / sqrt(2);
