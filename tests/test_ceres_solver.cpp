...
TEST (CeresSolver, HelloWorld1) {
    ...
    //Если посмотреть документацию, то 1) пишут что это для больших задач, тут явно не этот случай, 2) Нужна какая-та предпосчитаная штуковина, что за штуковина я как-то плохо поняла 3) Тут идёт переключение на неточный шаг, опять же не понятно зачем в такой простой задаче.
    //Да-да, ответ не то что бы обладает супер точностью, но так, что я поняла из быстрого прочтения обзаца текста.
    ...
    //Потому что у этой функции нет производной в этой точке. В общем там в одной стороны от этой точки производная -1, а с другой 1 и в этой точке какой-то конкретной
    // производной нет. Как такие функции называется? Я уже как-то забыла матан за 9 класс... не дифференцируемая функция.
