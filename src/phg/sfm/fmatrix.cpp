...
namespace {
    ...
    {
        int a_rows = count; //TODO - done
        int a_cols = 9; //TODO - done
...
//            std::cout << "(" << x0 << ", " << y0 << "), (" << x1 << ", " << y1 << ")" << std::endl;
        ...
        Eigen::VectorXd null_space = svda.matrixV().col(svda.matrixV().cols() - 1); //TODO - done
...
//          TODO - done
        Eigen::DiagonalMatrix<double, 3> S (svdf.singularValues().x(), svdf.singularValues().y(), 0);
    ...
    cv::Matx33d getNormalizeTransform(const std::vector<cv::Vec2d> &m)
    {
        ...
        for (auto i : m) {
            meanDistance += sqrt(pow(i[0] - mean[0], 2) + pow(i[1] - mean[1], 2));
        }
        meanDistance /= (sqrt(2) * m.size());
        return {1, 0, -mean[0],
                0, 1, -mean[1],
                0, 0, meanDistance
        };
...
//        // будет отличаться от случая с гомографией
        ...
        const int n_trials = phg::getCountTrials(n_samples, m0.size()); //TODO - done
