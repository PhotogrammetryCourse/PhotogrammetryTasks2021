...
//    // размерность всего 2, так что точное KD-дерево
    std::shared_ptr<cv::flann::IndexParams> index_params = flannKdTreeIndexParams(); // TODO - done
    std::shared_ptr<cv::flann::SearchParams> search_params = flannKsTreeSearchParams(); // TODO - done
...
//    // TODO заполнить filtered_matches - done
    ...
    for (int i = 0; i < n_matches; i++) {
        ...
        for (int j = 0; j < total_neighbours; j++) {
            if (distances2_query.at<float>(i, j) < radius2_query) {
                l_query.insert(indices_query.at<int>(i, j));
            }
            if (distances2_train.at<float>(i, j) < radius2_train) {
                l_train.insert(indices_train.at<int>(i, j));
            }
        }

        std::vector<int> intersection_ind;
        std::set_intersection(l_query.begin(), l_query.end(),
                              l_train.begin(), l_train.end(),
                              std::back_inserter(intersection_ind));
        if (intersection_ind.size() >= consistent_matches) {
