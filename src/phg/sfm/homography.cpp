...
namespace {
                                      ...
                                      const cv::Point2f &r2, const cv::Point2f &r3)
    {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            A.push_back({0, 0, 0, -x0 * w1, -y0 * w1, -w0 * w1, x0 * y1, y0 * y1, -w0 * y1}); // TODO - done
            A.push_back({x0 * w1, y0 * w1, w0 * w1, 0, 0, 0, -x0 * x1, -y0 * x1, w0 * x1}); // TODO - done
        }
    ...
    cv::Mat estimateHomographyRANSAC(const std::vector<cv::Point2f> &points_lhs, const std::vector<cv::Point2f> &points_rhs)
    {
        ...
        const int n_trials = 108; // TODO - done

        const int n_samples = 4; // TODO - done
        uint64_t seed = 1;
...
{
    cv::Vec3d point{pt.x, pt.y, 1};

    cv::Mat result = T * point;
    double w = result.at<double>(2);

    if (w == 0) {
        throw std::runtime_error("w = 0");
    }
    return {result.at<double>(0) / w, result.at<double>(1) / w};
}
