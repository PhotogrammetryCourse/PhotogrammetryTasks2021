...
//          TODO - done
        ...
        Eigen::VectorXd result = svda.matrixV().col(svda.matrixV().cols() - 1);
        cv::Mat resultCV(3, 4, CV_64F, result.data());
    ...
    cv::Matx34d estimateCameraMatrixRANSAC(const phg::Calibration &calib, const std::vector<cv::Vec3d> &X, const std::vector<cv::Vec2d> &x)
    {
        ...
        const int n_samples = 6; //TODO - done

        const int n_trials = phg::getCountTrials(n_samples, n_points); //TODO - done
        ...
        for (int i_trial = 0; i_trial < n_trials; ++i_trial) {
            ...
            for (int i = 0; i < n_samples; ++i) {
                ms0[i] = X[sample[i]]; //TODO - done
                ms1[i] = calib.unproject(x[sample[i]]); //TODO - done
            }
            ...
            for (int i = 0; i < n_points; ++i) {
                cv::Vec3d ray = calib.project(P * vector4d{X[i][0], X[i][1], X[i][2], 1});
                cv::Vec2d px = {ray[0] / ray[2], ray[1] / ray[2]};//TODO - done спроецировать 3Д точку в пиксель с использованием P и calib;
