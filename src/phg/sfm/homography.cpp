                                  ...
                                  const cv::Point2f &r2, const cv::Point2f &r3)
{
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        std::vector<double> row1 = {0., 0., 0., x0, y0, w0, -y1*x0, -y1*y0, y1};
        std::vector<double> row2 = {x0, y0, w0, 0., 0., 0.,  -x1*x0, -x1*y0, x1};
...
bool is_has_duplicate(const std::vector<cv::Point2f> &points_lhs, const std::vector<int>& sample){

    std::vector<cv::Point2f> set_pnts;

    set_pnts.reserve(sample.size());

    for(int id : sample){
        const cv::Point2f& pt = points_lhs[id];
        if(std::find_if(set_pnts.begin(), set_pnts.end(), [&pt](const cv::Point2f& p){
                        return std::fabs(p.x - pt.x) < 1e-5 && std::fabs(p.y - pt.y) < 1e-5 ;
    }) != set_pnts.end())
        {
            return true;
        }
        set_pnts.push_back(pt);
...
cv::Mat estimateHomographyRANSAC(const std::vector<cv::Point2f> &points_lhs, const std::vector<cv::Point2f> &points_rhs)
{
    ...
    for (int i_trial = 0; i_trial < n_trials; ++i_trial) {
        ...
        if(is_has_duplicate(points_lhs, sample))
            continue;

        if(is_has_duplicate(points_rhs, sample))
            continue;
...
{
    cv::Point2d out;
    double del = T.at<double>(2,0) * pt.x + T.at<double>(2,1) * pt.y + T.at<double>(2,2);
    out.x = (T.at<double>(0,0) * pt.x + T.at<double>(0,1) * pt.y + T.at<double>(0,2))/del;
    out.y = (T.at<double>(1,0) * pt.x + T.at<double>(1,1) * pt.y + T.at<double>(1,2))/del;
    return out;
}
