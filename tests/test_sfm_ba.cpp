...
// TODO -- done почему фокальная длина меняется от того что мы уменьшаем картинку? почему именно в такой пропорции? может надо домножать? или делить на downscale^2 ?
...
public:
    ...
    bool operator()(const T* camera_extrinsics, // положение камеры:   [6] = {translation[3], rotation[3]} (разное для всех кадров, т.к. каждая фотография со своего ракурса)
        ...
        const T* translation = camera_extrinsics; // - сдвиг в локальную систему координат камеры
        for (int i = 0; i < 3; i++) {
            point[i] = point_global[i] - translation[i];
        }

        T pointRot[3];
        const T* rotation = camera_extrinsics + 3; // - angle-axis rotation, поворачиваем точку point->p (чтобы перейти в локальную систему координат камеры)
        ...
        ceres::AngleAxisRotatePoint(rotation, point, pointRot);
        // Проецируем точку на фокальную плоскость матрицы (т.е. плоскость Z=фокальная длина), тем самым переводя в пиксели

        T x = camera_intrinsics[2]*pointRot[0]/pointRot[2];
        T y = camera_intrinsics[2]*pointRot[1]/pointRot[2];
#if ENABLE_INSTRINSICS_K1_K2

        T rr = x*x + y*y;

        x *= (T)1. + camera_intrinsics[0] * rr + camera_intrinsics[1] * rr * rr;
        y *= (T)1. + camera_intrinsics[0] * rr + camera_intrinsics[1] * rr * rr;
    ...
    for (size_t i = 0; i < tie_points.size(); ++i) {
        ...
        for (size_t ci = 0; ci < track.img_kpt_pairs.size(); ++ci) {
            ...
            if (ENABLE_OUTLIERS_FILTRATION_COLINEAR && ENABLE_BA && ci > 0) {
                ...
                vector3d camera;
                phg::decomposeUndistortedPMatrix(R, camera, cameras[0]);
                vector3d vectorF = cv::normalize(track_point-camera);
                vector3d vectorS = cv::normalize(track_point-camera_origin);
                if (vectorF.dot(vectorS) > std::cos(2.5*M_PI/180))
                    should_be_disabled = true;
            }
