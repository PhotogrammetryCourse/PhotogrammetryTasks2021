...
namespace phg {
    ...
    vector3d unproject(const vector3d &pixel, const phg::Calibration &calibration, const matrix34d &PtoWorld) {
        ...
        vector3d local_point = calibration.unproject({pixel[0], pixel[1]}); // #TODO 102 пустите луч pixel из calibration а затем возьмите ан нем точку у которой по оси +Z координата=depth
        vector4d local_point_with_depth = {local_point[0] * depth / local_point[2],
                                           local_point[1] * depth / local_point[2], depth, 1};
        vector3d global_point = PtoWorld * local_point_with_depth; // #TODO 103 переведите точку из локальной системы в глобальную

        return global_point;
    }
    std::string get_hyp_descr(int hi) {
        rassert(hi < 9, 3226426462862861);
        static std::vector<std::string> decs {
            "[d0, n0]", "[d0, nr]", "[d0, np]",
            "[dr, n0]", "[dr, nr]", "[dr, np]",
            "[dp, n0]", "[dp, nr]", "[dp, np]"
        };
    ...
    void PMDepthMapsBuilder::print_iter_stat() {
        ...
        verbose_cout << "stat for hypotheses: \n";
        for (const auto i: index) {
            verbose_cout << get_hyp_descr(i) << ": " << stat[i] << ", ";
        }
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    void PMDepthMapsBuilder::tryToPropagateDonor(ptrdiff_t ni, ptrdiff_t nj, int chessboard_pattern_step,
                                                 std::vector<float> &hypos_depth, std::vector<vector3f> &hypos_normal,
...
#pragma omp parallel for schedule(dynamic, 1)
            for (ptrdiff_t j = 0; j < height; ++j) {
                for (ptrdiff_t i = (j + chessboard_pattern_step) % 2; i < width; i += 2) {
                    ...
                    tryToPropagateDonor(i - 1 * PROPAGATION_STEP, j + 0 * PROPAGATION_STEP, chessboard_pattern_step,
                                        hypos_depth, hypos_normal, hypos_cost);
                    tryToPropagateDonor(i + 0 * PROPAGATION_STEP, j - 1 * PROPAGATION_STEP, chessboard_pattern_step,
                                        hypos_depth, hypos_normal, hypos_cost);
                    tryToPropagateDonor(i + 1 * PROPAGATION_STEP, j + 0 * PROPAGATION_STEP, chessboard_pattern_step,
                                        hypos_depth, hypos_normal, hypos_cost);
                    tryToPropagateDonor(i + 0 * PROPAGATION_STEP, j + 1 * PROPAGATION_STEP, chessboard_pattern_step,
    ...
    float PMDepthMapsBuilder::estimateCost(ptrdiff_t i, ptrdiff_t j, double d, const vector3d &global_normal,
                                           size_t neighb_cam) {
        ...
        for (ptrdiff_t dj = -COST_PATCH_RADIUS; dj <= COST_PATCH_RADIUS; ++dj) {
            for (ptrdiff_t di = -COST_PATCH_RADIUS; di <= COST_PATCH_RADIUS; ++di) {
                ...
                vector3d point_on_ray = unproject(vector3d(ni + 0.5, nj + 0.5, 1.0), calibration,
                                                  cameras_PtoWorld[ref_cam]);
                vector3d camera_center = unproject(vector3d(ni + 0.5, nj + 0.5, 0.0), calibration,
                                                   cameras_PtoWorld[ref_cam]); // TODO 204: это немного неестественный способ, можно поправить его на более явный вариант, например хранить центр камер в поле cameras_O
                ...
                // #TODO 205: замените этот наивный вариант nearest neighbor сэмплирования текстуры на билинейную интерполяцию
                //  (учтите что центр пикселя - .5 после запятой)

                double closest_int_x = std::round(x);
                double closest_int_y = std::round(y);

                cv::Point2d real_pixel{x, y};

                cv::Point2d p0{x - 0.5, y - 0.5};
                cv::Point2d p1{x + 0.5, y - 0.5};
                cv::Point2d p2{x + 0.5, y + 0.5};
                cv::Point2d p3{x - 0.5, y + 0.5};

                double w0 = 1.0 / cv::norm(real_pixel - p0);
                double w1 = 1.0 / cv::norm(real_pixel - p1);
                double w2 = 1.0 / cv::norm(real_pixel - p2);
                double w3 = 1.0 / cv::norm(real_pixel - p3);
                double WW = w0 + w1 + w2 + w3;
                ...
                ptrdiff_t u2 = closest_int_x;
                ptrdiff_t u1 = u2 - 1;
                ptrdiff_t v2 = closest_int_y;
                ptrdiff_t v1 = v2 - 1;

                // #TODO 108: добавьте проверку "попали ли мы в камеру номер neighb_cam?" если не попали - возвращаем NO_COST
                if (!(u2 >= 0 && u1 > 0 && u2 < width && u1 < width && v2 >= 0  && v1 >= 0 && v2 < height && v2 < height))
                    return NO_COST;

                float intensity = cameras_imgs_grey[neighb_cam].at<unsigned char>(v1, u1) * w0 / 255.0f
                        + cameras_imgs_grey[neighb_cam].at<unsigned char>(v1, u2) * w1 / 255.0f
                        + cameras_imgs_grey[neighb_cam].at<unsigned char>(v2, u2) * w2 / 255.0f
                        + cameras_imgs_grey[neighb_cam].at<unsigned char>(v2, u1) * w3 / 255.0f
                        ;
        ...
        // f(x, y) -> F(x, y) как в википедии
        for (size_t k = 0; k < n ; ++k) {
            patch0[k] -= mean0;
            patch1[k] -= mean1;
        }

        float p0_norm = cv::norm(patch0);
        float p1_norm = cv::norm(patch1);

        if (p0_norm == 0|| p1_norm == 0)
            return NO_COST;

        float zncc = 0.0f;
        for (size_t k = 0; k < n; ++k) {
            zncc += (patch0[k] * patch1[k]) / (p0_norm * p1_norm);
        }
    ...
    float PMDepthMapsBuilder::avgCost(std::vector<float> &costs) {
        ...
        float cost_w = 1.0f;
        float threshold = best_cost * COSTS_K_RATIO;
        size_t k_limit = costs.size() < COSTS_BEST_K_LIMIT ? costs.size() : COSTS_BEST_K_LIMIT;
        ...
        // Не очень понятно что такое большой, но у нас есть некоторая контанта GOOD_COST, давайте ужесточим порог, если
        // best_cost больше GOOD_COST
        if (best_cost > GOOD_COST) threshold *= 0.5;
...
#pragma omp parallel for schedule(dynamic, 1) reduction(+:costs_sum, costs_n, good_costs_sum, good_costs_n)
        ...
        verbose_cout << to_percent(good_costs_n, ntotal) << "% pixels with good " << (good_costs_sum / good_costs_n)
                     << " avg cost";
        verbose_cout << std::endl;
    }
    ...
    void PMDepthMapsBuilder::debugCurrentPoints(const std::string &label) {
        ...
        exportPointCloud(point_cloud_all, DEBUG_DIR + label + "_all_rgb_bilin.ply", point_cloud_all_bgr,
                         point_cloud_all_normal);
        exportPointCloud(point_cloud_good, DEBUG_DIR + label + "_good_rgb_bilin.ply", point_cloud_good_bgr,
                         point_cloud_good_normal);
        exportPointCloud(point_cloud_good, DEBUG_DIR + label + "_good_costs_bilin.ply", point_cloud_good_cost,
                         point_cloud_good_normal);
    }
}
