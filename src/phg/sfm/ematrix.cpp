...
// TODO - done
        Eigen::DiagonalMatrix<double, 3> diagonalMatrix(1, 1, 0);
        E = svd.matrixU() * diagonalMatrix * svd.matrixV().transpose();
...
{
    matrix3d E = calib1.K().t() * F * calib0.K(); //TODO - done
...
//        // скомпенсировать калибровки камер
        vector3d p0 = calib0.unproject(m0); //TODO - done
        vector3d p1 = calib1.unproject(m1); //TODO - done
...
//        // точка должна иметь положительную глубину для обеих камер
        return p0.dot(P0 * X) > 0 && p1.dot(P1 * X) > 0; //TODO - done
    }
}
...
void phg::decomposeEMatrix(cv::Matx34d &P0, cv::Matx34d &P1, const cv::Matx33d &Ecv, const std::vector<cv::Vec2d> &m0, const std::vector<cv::Vec2d> &m1, const Calibration &calib0, const Calibration &calib1)
{
    ...
    mat R0 = U * W * V.transpose(); //TODO - done
    mat R1 = U * W.transpose() * V.transpose(); //TODO - done

    std::cout << "R0:\n" << R0 << std::endl;
    std::cout << "R1:\n" << R1 << std::endl;

    vec t0 = U * (Eigen::Vector3d (0, 0, 1)); //TODO - done
    vec t1 = -t0; //TODO - done
