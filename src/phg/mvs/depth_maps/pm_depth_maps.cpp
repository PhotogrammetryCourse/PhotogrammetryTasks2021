...
//    std::cout << "The ray intersects the plane at " << ip << std::endl;
    ...
    vector3d global_point = PtoWorld * homogenize(lp); // TODO 103 переведите точку из локальной системы в глобальную
...
float PMDepthMapsBuilder::estimateCost(ptrdiff_t i, ptrdiff_t j, double d, const vector3d &global_normal, size_t neighb_cam)
{
    ...
    for (ptrdiff_t dj = -COST_PATCH_RADIUS; dj <= COST_PATCH_RADIUS; ++dj) {
        for (ptrdiff_t di = -COST_PATCH_RADIUS; di <= COST_PATCH_RADIUS; ++di) {
            ...
            if(v < 0 || u < 0 || cameras_imgs_grey[neighb_cam].cols < v || cameras_imgs_grey[neighb_cam].rows < u)
                return NO_COST;
    ...
    float sum_all = 0., sum_pow_1 = 0., sum_pow_2 = 0.;
    for (size_t k = 0; k < n; ++k) {
...
float PMDepthMapsBuilder::avgCost(std::vector<float> &costs)
{
    ...
    cost_sum = std::accumulate(costs.begin(), costs.size() <  COSTS_BEST_K_LIMIT ? costs.end() : costs.begin() + COSTS_BEST_K_LIMIT, 0., [&best_cost, &cost_w](float a, float v){
        if(v < best_cost * COSTS_K_RATIO)
        {
            cost_w +=1.;
            return a + v;
        }
