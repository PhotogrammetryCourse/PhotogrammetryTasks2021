...
{
    cv::Vec2d center_mass;
    /*находим центр масс, для каждой из точек ищем расстояние до центра масс*/

    cv::Matx33d nt33 = cv::Matx33d::eye();
    size_t count = m.size();
    ...
    double dist = 0;
    for(const cv::Vec2d& v : m){
        dist += (std::sqrt(std::pow(v(0) - nt33(0,2),2) + std::pow(v(1) - nt33(1,2),2)) / count);
    }

    nt33(0,2) *= (-std::sqrt(2) / dist);
    nt33(1,2) *= (-std::sqrt(2) / dist);

    nt33(0,0) = std::sqrt(2) / dist;
    nt33(1,1) = std::sqrt(2) / dist;
