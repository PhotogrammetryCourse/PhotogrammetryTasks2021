...
namespace {
    ...
    void ensureSpectralProperty(matrix3d &Ecv)
    {
        ...
        E = svd.matrixU() * matrix_s * svd.matrixV().transpose();
...
// существенная матрица (Essential matrix = K1^t * F * K0) имеет ровно два совпадающих ненулевых сингулярных значения, тогда как для фундаментальной матрицы они могут различаться
