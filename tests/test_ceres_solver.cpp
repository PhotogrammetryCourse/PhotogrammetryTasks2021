...
public:
    ...
    bool operator()(const T* const line, T* residual) const {
        ...
        if (line[0] * line[0] + line[1] * line[1] == T(0))
            return false;

        residual[0] = 
            (line[0] * T(samplePoint[0]) + line[1] * T(samplePoint[1]) + line[2]) // signed distance * normal vector
            / 
            ceres::sqrt(line[0] * line[0] + line[1] * line[1]);                   // divide by the normal vector to get real distance
        return true;
    }
protected:
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    const double ideal_line[3] = {0.5, -1, 100.0}; // 0.5x - y + 100 = 0

    std::cout << "Ideal line: (a=" << ideal_line[0] << ", b=" << ideal_line[1] << ", c=" << ideal_line[2] << ")" << std::endl;
    ...
    // Создаем единственный блок параметров: [a, b, c] - прямая которую мы оптимизируем
    ...
    double threshold = 2e-3 * std::max({std::abs(ideal_line[0]), std::abs(ideal_line[1]), std::abs(ideal_line[2])}); // пришлось чуть расширить порог(
...

    {
        ...
        for (int dim = 0; dim < 3 && !has_q; ++dim) {
            if (std::abs(line_params[dim]) > threshold) {
                q = ideal_line[dim] / line_params[dim];
                has_q = true;
            }
        }
        ...
        for (int dim = 0; dim < 3; ++dim) {
            ASSERT_NEAR(ideal_line[dim], line_params[dim] * q, threshold);
        }
    }
