...
public:
    ...
    bool operator()(const T* const queryPoint, T* residual) const {
        ...
        // тут не должен быть корень, чтобы это действительно было расстояние, а не квадрат расстояния?
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    std::cout << summary.BriefReport() << std::endl;
    // пост-обработка (если не менять формулировку задачи для ceres-solver)
    double coef = ideal_line[2] / line_params[2];
    for (int d = 0; d < 3; ++d) {
        line_params[d] *= coef;
    }
    ...
    if (outliers_fraction > 0.0 && !use_huber) {
        // без увеличения коэффициента, на который домножаем, не получается пройти CeresSolver.FitLineNoiseAndOutliers
