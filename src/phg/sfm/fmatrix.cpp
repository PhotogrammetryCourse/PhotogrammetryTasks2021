...
//        std::cout << "matrixV: \n" << svda.matrixV() << std::endl;
//        std::cout << "singularValues: \n" << svda.singularValues() << std::endl;
        Eigen::VectorXd right_null_space = svda.matrixV().col(8);

//        std::cout << "right_null_space: \n" << right_null_space << std::endl;

        Eigen::MatrixXd F(3, 3);
        F.row(0) << right_null_space[0], right_null_space[1], right_null_space[2];
        F.row(1) << right_null_space[3], right_null_space[4], right_null_space[5];
        F.row(2) << right_null_space[6], right_null_space[7], right_null_space[8];
...
//             Поправить F так, чтобы соблюдалось свойство фундаментальной матрицы (последнее сингулярное значение = 0)
    ...
    {
        // общее изменение масштаба -- последний диагональный элемент.
        // если T:
        //    1 0 x_shift
        //    0 1 y_shift
        //    0 0 scale

        // то T * (x, y, 1) = (x + x_shift, y + y_shift, scale)
        ...
        double dist = 0;
        for (const auto & pt : m) {
            double diff_x = pt[0] - center.x;
            double diff_y = pt[1] - center.y;
            dist += std::sqrt(diff_x * diff_x + diff_y * diff_y);
        }
        dist /= m.size();

        return {1.0, 0.0, -center.x,
                0.0, 1.0, -center.y,
                0.0, 0.0, dist / std::sqrt(2)};
    }
    ...
    cv::Matx33d estimateFMatrixRANSAC(const std::vector<cv::Vec2d> &m0, const std::vector<cv::Vec2d> &m1, double threshold_px)
    {
        ...
        {
//            double dist0 = 0;
//            for (const cv::Vec2d & pt: m0_t) {
//                dist0 += std::sqrt(pt[0] * pt[0] + pt[1] * pt[1]);
//            }
//            std::cout << "avg dist0: " << dist0 / m0_t.size() << std::endl;
//
//            double dist1 = 0;
//            for (const cv::Vec2d & pt: m1_t) {
//                dist1 += std::sqrt(pt[0] * pt[0] + pt[1] * pt[1]);
//            }
//            std::cout << "avg dist1: " << dist1 / m1_t.size() << std::endl;
...
//             Проверьте лог: при повторной нормализации должно найтись почти единичное преобразование
            std::cout << getNormalizeTransform(m0_t) << std::endl;
            std::cout << getNormalizeTransform(m1_t) << std::endl;
        }
...
//      // будет отличаться от случая с гомографией
        ...
        for (int i_trial = 0; i_trial < n_trials; ++i_trial) {
            ...
            int support = 0;
            for (int i = 0; i < n_matches; ++i) {
                if (phg::epipolarTest(m0[i], m1[i], F, threshold_px) && phg::epipolarTest(m1[i], m0[i], F_tr, threshold_px))
                {
                    ++support;
                }
            }
